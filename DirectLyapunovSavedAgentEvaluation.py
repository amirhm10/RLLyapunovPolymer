...
def plot_tracking(bundles: dict[str, dict], output_path: Path) -> None:
    ...
    y_sp_phys = y_sp_phys_from_bundle(first_bundle)
    ...
    for idx, ax in enumerate(axes):
        ...
        for case_name, bundle in bundles.items():
            y = np.asarray(bundle["y_system"], dtype=float)
            ax.plot(t[: y.shape[0]], y[:, idx], linewidth=1.2, label=case_name)
