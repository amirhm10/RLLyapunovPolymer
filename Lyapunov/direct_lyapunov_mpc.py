...
class DirectOutputDisturbanceLyapunovMpcSolver(FirstStepContractionTrackingLyapunovMpcSolver):
    ...
    def solve_tracking_mpc_step(
        ...
        use_soft_slack = bool(lyapunov_mode == "soft" and active_first_step_contraction)
        ...
        for solver_name in solver_sequence:
            try:
                ...
                if use_soft_slack and lyap_slack is not None:
                ...
                if problem.status in _OPTIMAL_STATUSES and last_eval["accepted"]:
                    return SimpleNamespace(
                        ...
                        relaxed_contraction_satisfied=
                        last_eval["relaxed_contraction_satisfied"]
                        if use_soft_slack
                        else last_eval["first_step_contraction_satisfied"],
                        relaxed_contraction_violation=
                        last_eval["relaxed_contraction_violation"]
                        if use_soft_slack
...
def make_direct_lyapunov_step_records(step_info_storage):
    ...
    for info in step_info_storage:
        row = {
            ...
            "report_window": info.get("report_window"),
            "step_in_report_window": info.get("step_in_report_window"),
            "phase_step": info.get("phase_step"),
            "phase1_episode": info.get("phase1_episode"),
            "phase2_report_window": info.get("phase2_report_window"),
            ...
            "reward_tracking_cost": info.get("reward_tracking_cost"),
            "reward_move_cost": info.get("reward_move_cost"),
            "reward_bonus": info.get("reward_bonus"),
            "reward_w_in": info.get("reward_w_in"),
...
def summarize_direct_lyapunov_bundle(bundle):
    ...
    summary = {
        ...
        "dx_s_inf_mean": _safe_nanmean(bundle.get("dx_s_inf", [])),
        "dx_s_inf_max": _safe_nanmax(bundle.get("dx_s_inf", [])),
        "dc_rate_inf_mean": _safe_nanmean(bundle.get("dc_rate_inf", [])),
        "dc_rate_inf_max": _safe_nanmax(bundle.get("dc_rate_inf", [])),
        "dx_s_max_active_rate": _safe_nanmean(bundle.get("dx_s_max_active_flags", [])),
        "dx_s_max_inf_mean": _safe_nanmean(bundle.get("dx_s_max_inf", [])),
...
def make_direct_lyapunov_phase_records(bundle) -> List[Dict[str, Any]]:
    ...
    for window in windows:
        ...
        row: Dict[str, Any] = {
            ...
            "report_window_start": window.get("report_window_start"),
            "report_window_end": window.get("report_window_end"),
            "learning_episode_start": window.get("learning_episode_start"),
            "learning_episode_end": window.get("learning_episode_end"),
            "phase2_report_window_start": window.get("phase2_report_window_start"),
            "phase2_report_window_end": window.get("phase2_report_window_end"),
            "step_start": int(start),
            "step_stop_exclusive": int(stop),
            "n_steps": int(stop - start),
            "reward_mean": _safe_nanmean(rewards[start:stop]),
...
def make_direct_lyapunov_comparison_record(case_name, bundle, debug_dir=None):
    ...
    record = {
        ...
        "dx_s_inf_mean": summary.get("dx_s_inf_mean"),
        "dx_s_inf_max": summary.get("dx_s_inf_max"),
        "dc_rate_inf_mean": summary.get("dc_rate_inf_mean"),
        "dc_rate_inf_max": summary.get("dc_rate_inf_max"),
        "dx_s_max_active_rate": summary.get("dx_s_max_active_rate"),
        "dx_s_max_inf_mean": summary.get("dx_s_max_inf_mean"),
...
def build_direct_lyapunov_run_bundle(
    ...
    *,
    ...
    bundle = {
        ...
        "reward_tracking_cost": np.array([info.get("reward_tracking_cost", np.nan) for info in direct_info_storage], dtype=float),
        "reward_move_cost": np.array([info.get("reward_move_cost", np.nan) for info in direct_info_storage], dtype=float),
        "reward_bonus": np.array([info.get("reward_bonus", np.nan) for info in direct_info_storage], dtype=float),
        "reward_w_in": np.array([info.get("reward_w_in", np.nan) for info in direct_info_storage], dtype=float),
        ...
        "report_window": np.array([info.get("report_window", np.nan) for info in direct_info_storage], dtype=float),
        "step_in_report_window": np.array(
            [info.get("step_in_report_window", np.nan) for info in direct_info_storage],
            dtype=float,
        ),
        "phase_step": np.array([info.get("phase_step", np.nan) for info in direct_info_storage], dtype=float),
        "phase1_episode": np.array([info.get("phase1_episode", np.nan) for info in direct_info_storage], dtype=float),
        "phase2_report_window": np.array(
            [info.get("phase2_report_window", np.nan) for info in direct_info_storage],
            dtype=float,
        ),
        ...
        "target_rate_x_inf": np.array([info.get("target_rate_x_inf", info.get("dx_s_inf", np.nan)) for info in direct_info_storage], dtype=float),
        "dx_s_inf": np.array([info.get("dx_s_inf", info.get("target_rate_x_inf", np.nan)) for info in direct_info_storage], dtype=float),
        "dc_rate_inf": np.array([info.get("dc_rate_inf", info.get("d_cert_delta_inf", np.nan)) for info in direct_info_storage], dtype=float),
        "dx_s_max_active_flags": np.array([1.0 if bool(info.get("dx_s_max_active", False)) else 0.0 for info in direct_info_storage], dtype=float),
        "dx_s_max_inf": np.array([info.get("dx_s_max_inf", np.nan) for info in direct_info_storage], dtype=float),
