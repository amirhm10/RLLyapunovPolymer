...
def run_standard_tracking_lyapunov_mpc_first_step_contraction(
    system,
    LMPC_obj,
    y_sp_scenario,
    n_tests,
    ...
    for step_idx in range(nFE):
        ...
        if bool(dbg_tgt.get("success", False)) and x_s is not None and u_s is not None:
            ...
            step_info.update({
                ...
                "terminal_constraint_skipped": terminal_constraint_skipped,
                "y_target": y_target.copy(),
                "V_k": getattr(sol, "V_k", None),
                "V_next_first": getattr(sol, "V_next_first", None),
                "V_bound": getattr(sol, "V_bound", None),
                "contraction_margin": getattr(sol, "contraction_margin", None),
                "first_step_contraction_satisfied": getattr(sol, "first_step_contraction_satisfied", None),
                "contraction_constraint_violation": getattr(sol, "contraction_constraint_violation", None),
            })
            ...
            if getattr(sol, "success", False):
                ...
                step_info.update({
                    "success": True,
                    "method": "standard_tracking_lyapunov_mpc_first_step_contraction",
                    "u_apply": u_dev_apply.copy(),
                    **report,
                })
                step_info["alpha_terminal_raw"] = float(alpha_terminal_raw)
