...
def make_safety_filter_step_records(lyap_info_storage):
    ...
    for step_idx, info in enumerate(lyap_info_storage):
        ...
        row = {
            ...
            "behavior_exploration_space": info.get("behavior_exploration_space"),
...
def build_safety_filter_run_bundle(
    ...
    bundle = {
        ...
        "report_window": np.array([info.get("report_window", np.nan) for info in lyap_info_storage], dtype=float),
        "step_in_report_window": np.array(
            [info.get("step_in_report_window", np.nan) for info in lyap_info_storage],
            dtype=float,
        ),
        "phase_step": np.array([info.get("phase_step", np.nan) for info in lyap_info_storage], dtype=float),
        "phase1_episode": np.array([info.get("phase1_episode", np.nan) for info in lyap_info_storage], dtype=float),
        "phase2_report_window": np.array(
            [info.get("phase2_report_window", np.nan) for info in lyap_info_storage],
            dtype=float,
        ),
        ...
        "teacher_input_exploration_requested_store": _stack_vectors(
            lyap_info_storage,
            "teacher_input_exploration_requested",
            n_u,
        ),
        "teacher_input_exploration_applied_store": _stack_vectors(
            lyap_info_storage,
        ...
        "reward_tracking_cost": np.array([info.get("reward_tracking_cost", np.nan) for info in lyap_info_storage], dtype=float),
        "reward_move_cost": np.array([info.get("reward_move_cost", np.nan) for info in lyap_info_storage], dtype=float),
        "reward_bonus": np.array([info.get("reward_bonus", np.nan) for info in lyap_info_storage], dtype=float),
        "reward_w_in": np.array([info.get("reward_w_in", np.nan) for info in lyap_info_storage], dtype=float),
...
def _safety_npz_arrays(bundle, export_profile="debug"):
    ...
    arrays = {
        ...
        "report_window": bundle.get("report_window", np.full(int(bundle.get("nFE", 0)), np.nan)),
        "step_in_report_window": bundle.get(
            "step_in_report_window",
            np.full(int(bundle.get("nFE", 0)), np.nan),
        ),
        "phase_step": bundle.get("phase_step", np.full(int(bundle.get("nFE", 0)), np.nan)),
        "phase1_episode": bundle.get("phase1_episode", np.full(int(bundle.get("nFE", 0)), np.nan)),
        "phase2_report_window": bundle.get(
            "phase2_report_window",
            np.full(int(bundle.get("nFE", 0)), np.nan),
        ),
