import numpy as np
from utils.scaling_helpers import apply_min_max, reverse_min_max, apply_rl_scaled
from utils.helpers import generate_setpoints_training_rl_gradually


def run_rl_pre_trained(system, agent, MPC_obj, y_sp_scenario, n_tests, set_points_len,
                       ...
                       min_max_dict,
                       mode="disturb"):
    # warm_start here only affects the train/test schedule helper; it is not
    # an MPC takeover or control warm-start mechanism.
    ...
    for i in range(nFE):
        ...
        # scale & step plant
        u_mpc[i, :] = u_scaled + ss_scaled_inputs
...
def run_rl_train(system, y_sp_scenario, n_tests, set_points_len,
    ...
    # warm_start here only affects when online TD3 updates start; it is not an
    # MPC takeover or deterministic-control warm-start mechanism.
    # --- setpoints generation ---
    y_sp, nFE, sub_episodes_changes_dict, time_in_sub_episodes, test_train_dict, WARM_START, qi, qs, ha = \
        generate_setpoints_training_rl_gradually(
            y_sp_scenario, n_tests, set_points_len, warm_start, test_cycle,
