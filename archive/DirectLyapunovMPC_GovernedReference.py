# Proposal 01 governed-reference direct Lyapunov MPC test runner.
#
# This script mirrors DirectLyapunovMPC.py, but it uses an explicit
# governed command layer:
#
#     y_sp,k -> r_cmd,k -> (x_s, u_s, y_s)
#
# The current bounded/unbounded target selectors are not changed by this
# runner. Results are saved separately under results/LyapMPC_GovernedReference.
...
#
# This run isolates Proposal 01. The raw setpoint is first mapped to an
# admissible governed command `r_cmd`. The steady Lyapunov target is then
# computed around `r_cmd`, while closed-loop performance is still evaluated
# against the raw setpoint unless `use_target_output_for_tracking` is changed.
...
study_name = "LyapMPC_GovernedReference"
...
governed_reference_target_config = {
...
case_specs = [
    {
        "case_name": "lyap_mpc_governed_reference",
        "target_mode": "governed_reference",
        "lyapunov_mode": "hard",
        "target_config": dict(governed_reference_target_config),
        "controller_mode": "direct_lyapunov_mpc",
        "label": "Governed-reference LyapMPC",
    }
]

print("Effective governed-reference direct Lyapunov MPC case specs.")
...
def run_case(case_spec):
    ...
    case_lyapunov_mode = case_spec.get("lyapunov_mode", "hard")
...
print("Saved governed-reference direct Lyapunov MPC study:")
