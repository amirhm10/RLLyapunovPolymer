...
def _normalize_tracking_target_policy(selector_mode, mpc_target_policy, tracking_target_policy):
    selector_mode = str(selector_mode)
    if selector_mode == "compromised_reference":
        return "selector_reference"
    if tracking_target_policy is None and selector_mode in {"free_disturbance_prior", "single_stage_robust_sstp"}:
        return "selector_reference"
...
def _target_selector_overrides(target_selector_config, u_nom_tgt, Qs_tgt_diag, Ru_tgt_diag, w_x_tgt, Qdx_tgt_diag, Rmove_diag, target_solver_pref):
    ...
    overrides.update(_as_selector_config_dict(target_selector_config))
    return overrides


def _selector_decomposition(C_aug, n_x, target_info, d_hat):
    ...
    cx = np.asarray(C @ x_s, float).reshape(-1)
    cdd = np.asarray(Cd @ d_s, float).reshape(-1)
    return cx, cdd
...
def run_mpc_lyapunov(
    ...
    for k in range(nFE):
        ...
        if use_lyap:
            ...
            info["selector_Qd_diag_used"] = selector_dbg.get("Qd_diag_used")
            ...
            info = {
                ...
                "selector_Qd_diag_used": target_info.get("selector_debug", {}).get("Qd_diag_used"),
