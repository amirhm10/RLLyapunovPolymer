...
The terminal constraint supports the finite-horizon MPC design and can improve closed-loop regularity, but it is not the minimal ingredient in the moving-target Lyapunov proof below. The proof uses the hard one-step contraction of the executed input. Therefore, for RL exploration, the terminal constraint may be omitted from the safety projection as long as the executed input still satisfies the same hard one-step GART contraction inequality.
...
## 12.1 Assumptions For The Stability Statement

The following assumptions define the conditions under which the moving-target practical-stability statement is claimed.

**Assumption 1: Valid Lyapunov matrix.**
The matrix $P_x$ used in the target probe and in the GART-LMPC first-step constraint satisfies $P_x\succ 0$. The Lyapunov function is

$$
V(e)=e^\top P_x e .
$$

**Assumption 2: Accepted target sequence.**
At each certified sampling instant $k$, the target selector returns an accepted target

$$
(x_{s,k},u_{s,k},y_{s,k})
$$

satisfying the equilibrium and certified-disturbance target equations

$$
x_{s,k}=Ax_{s,k}+Bu_{s,k},
\qquad
y_{s,k}=Cx_{s,k}+d^c_k .
$$

The target-motion constraint is enforced:

$$
\|x_{s,k+1}-x_{s,k}\|_\infty\le d_{x_s}.
$$

**Assumption 3: Certified executed input.**
The input applied to the plant or observer model at time $k$, denoted $u_k^{\mathrm{exec}}$, satisfies the input constraints and the hard one-step target-centered Lyapunov contraction inequality
...
$$
x_{k+1}=Ax_k+Bu_k^{\mathrm{exec}} .
$$

For the pure GART-LMPC controller, $u_k^{\mathrm{exec}}$ is the first input returned by a feasible and successfully solved GART-LMPC problem. For the RL-safety variant, $u_k^{\mathrm{exec}}$ is the projected safe action that satisfies the same hard one-step contraction inequality.

**Assumption 4: Successful certified step.**
The practical-stability recursion is claimed only on sampling instants for which an accepted target exists and the executed input satisfies Assumption 3. If no target is usable or if the finite-horizon GART-LMPC optimization fails and the implementation holds the previous input, that fallback step is not counted as a certified Lyapunov-contraction step unless the held input itself satisfies the same contraction inequality.

**Assumption 5: Bounded model/estimator residual for external tracking.**
For the raw-setpoint tracking interpretation, the residual term $\varepsilon_{\mathrm{est},k}$, which collects estimator mismatch, plant-model mismatch, and certified/raw disturbance mismatch, is assumed bounded.

## 13. Moving-Target Practical Stability Bound

**Theorem 1: Practical target-centered stability of GART-LMPC.**
Suppose Assumptions 1--4 hold. Let

$$
e_k=x_k-x_{s,k}
$$

and let the accepted target sequence satisfy

$$
\|x_{s,k+1}-x_{s,k}\|_\infty\le d_{x_s}.
$$

Choose any $\mu>0$ such that

$$
\bar\rho=(1+\mu)\rho<1 .
$$

Then the target-centered Lyapunov value satisfies the practical recursion
...
On any certified sampling instant satisfying Assumptions 2--4, the executed input contracts toward the target used at time $k$:
...
The bound above is a certificate of practical boundedness, not a tight performance prediction. For the reported value $\rho=0.98$, the proof parameter must satisfy
...
Thus the multiplier $1+1/\mu$ in $w$ can be large. This conservatism comes from the worst-case bound on the moving target displacement and does not imply that the observed closed-loop tracking error will be large. The empirical closed-loop tracking metrics should therefore be used as the tighter performance assessment, while the analytic tube is used as a qualitative stability certificate.
...
**Corollary 1: Raw-setpoint tracking interpretation.**
Suppose the conditions of Theorem 1 hold and Assumption 5 holds. Then the raw setpoint tracking error admits the decomposition
...
Consequently, the raw tracking error is bounded by the target-centered stability tube, the target-to-setpoint displacement, and the residual estimator/model mismatch.
...
Combining Theorem 1 with Corollary 1 gives the external tracking interpretation
...
where $x_{k+1}(u)$ is the one-step state prediction under input $u$.

**Proposition 1: RL exploration under the GART safety projection.**
Suppose the target selector returns an accepted target at time $k$. Let the RL actor propose $u_k^{\mathrm{RL}}$, but let the plant receive the projected safe input $u_k^{\mathrm{safe}}$. If the projection problem is feasible and returns an input satisfying

$$
u_k^{\mathrm{safe}}\in\mathcal U
$$
...
then the same one-step contraction condition used in Theorem 1 holds for the executed input. Therefore, the moving-target practical-stability recursion is preserved during RL exploration, provided the target-motion bound also holds.

The terminal constraint is not required for this one-step RL safety certificate. It may still be used in the finite-horizon GART-LMPC expert controller, but the minimal condition for RL exploration is that the executed input satisfies the hard one-step GART contraction constraint.

If the actor proposal already satisfies the input constraints and the Lyapunov contraction constraint, then
...
| $u_k^{\mathrm{exec}}$ | Input executed by the plant or observer model | GART-LMPC first move or projected safe RL input | Must satisfy the hard one-step contraction condition on certified steps |
